...
export default function SwapScreen() {
  ...
  React.useEffect(() => {
    ...
    if (!isNaN(amount)) {
      ...
      } else if (payToken.symbol === 'ETH' && receiveToken.symbol === 'BTC') {
        setReceiveAmount((amount / exchangeRate).toFixed(4));
      }
    } else {
      setReceiveAmount('');
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="flex justify-center">
          <button onClick={handleReverse}>
            <CircularIconContainer icon={<Reverse className="w-6 h-6" />} />
          </button>
        </div>
        ...
        <p className="text-center text-sm text-gray-400">
          1 {payToken.symbol} ~= {payToken.symbol === 'BTC' ? exchangeRate : (1 / exchangeRate).toFixed(4)} {receiveToken.symbol}
        </p>
